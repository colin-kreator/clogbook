$(document).ready(function() {
	...
	$('.ifr-time input').on('flightTimeOk',function(){
		$('#IFR_CB').prop('checked', true);
	});
...
function recordFlight() {
	...
	const flightDto = createJsonToSend();
	console.log(flightDto);

	const checker = $("#modal_newflight .is-danger:visible");
	if (checker.length > 0) {
		console.log($("#invalid_flight_modal_message").text());
		alert($("#invalid_flight_modal_message").text());
	} else {
		$.ajax({
			type: "POST",
			...
			.done(function() {
				window.location.replace(CONTEXT_PATH + "app/dashboard");
			})
			.fail(function() {
				alert("DTC");
			}).then(function() {
...
function createJsonToSend() {
	...
	//ID
	if ($('#flight_id_input').val()) {
		json["id"] = $('#flight_id_input').val()
	}

	//DATE
	const j_date = $("#flight_date input").val();
	const dd = j_date.split("/")[0];
	const mm = j_date.split("/")[1];
	const yy = yyyy(j_date.split("/")[2]);
...
function timeInHrMin(timeInMin) {
	const hours =  (timeInMin - timeInMin % 60) / 60+'';
	...
	$.ajax({
		...
		.done(function(response) {
			for (plt of response) {
				...
				if(plt.firstName != null && plt.lastName != null){
					$("#pic_datalist").append($("<option>").attr('value', plt.firstName + " " + plt.lastName).data("plt", plt));
				}else if(plt.firstName == null && plt.lastName != null){
					$("#pic_datalist").append($("<option>").attr('value', plt.lastName).data("plt", plt));
				}if(plt.firstName != null && plt.lastName == null){
					$("#pic_datalist").append($("<option>").attr('value', plt.firstName).data("plt", plt));
				}
...
function validateFlightTimeElement(element) {
	...
	if (element.val().length == 0) {
		...
		if (isTimeValid(element.val())) {
			element.removeClass("is-danger");
			element.addClass("is-success").trigger('flightTimeOk');
		} else {
			element.addClass("is-danger");
			element.removeClass("is-success");
		}
...
const yyyy = yy => {
	const zzzz = '20' + yy;
	const xxxx = '19' + yy;
	if (zzzz > new Date().getUTCFullYear()+1) {
		console.log(xxxx);
		return xxxx;
	} else {
		console.log(zzzz);
...
function resetForm() {
	...
	$('#flight_aircraft .type input').data("am", null);
	$('#flight_aircraft .type input').prop('disabled',false);
	$('#flight_pic input').data("plt", null);
	$('#flight_aircraft .reg input').data("act", null);
	...
	$('#actualFlight').prop('checked', true).click();
}

function resetFormWithFlight(tr) {
	...
	//DATE
	const date = flt.date.split('/');
	
	$("#flight_date input").val(date[0]+'/'+date[1]+'/'+date[2].substring(2)).trigger('input');

	//AIRCRAFT
	if (flt.aircraft != null && flt.aircraft.aircraftModel != null)
		$('#flight_aircraft .type input').val(flt.aircraft.aircraftModel.customName).change();
	if (flt.aircraft != null)
		$("#flight_aircraft .reg input").val(flt.aircraft.registration).change();

	//AIRPORTS
	if (flt.departureAirport != null)
		$("#flight_departure .place input").val(flt.departureAirport.icaoCode).change();

	if (flt.arrivalAirport != null)
		$("#flight_arrival .place input").val(flt.arrivalAirport.icaoCode).change();

	//TIMES OF ARRIVAL / DEPARTURE
	$("#flight_departure .time input").val(flt.departureTime).trigger('input');
	$("#flight_arrival .time input").val(flt.arrivalTime).trigger('input');

	if (flt.pilotDto != null && flt.pilot != null) {
		if (flt.pilotDto.lastName == 'Self') {
			flt.pilot.lastName="(" + flt.pilot.firstName + " " + flt.pilot.lastName + ")";
			flt.pilot.firstName="Self";
		}
		if(flt.pilot.firstName != null && flt.pilot.lastName != null){
			$('#flight_pic input').val(flt.pilot.firstName + " " + flt.pilot.lastName).change();
		}else if(flt.pilot.firstName == null && flt.pilot.lastName != null){
			$('#flight_pic input').val(flt.pilot.lastName).change();
		}if(flt.pilot.firstName != null && flt.pilot.lastName == null){
			$('#flight_pic input').val(flt.pilot.firstName).change();
		}
		
	}

	$('#flight_to-ldg .to-day input').val(flt.dayTO).trigger('input');
	$('#flight_to-ldg .to-night input').val(flt.nightTO).trigger('input');
	$('#flight_to-ldg .ldg-day input').val(flt.dayLdg).trigger('input');
	$('#flight_to-ldg .ldg-night input').val(flt.nightLdg).trigger('input');
	
	if(flt.multiPilot !=null ) $('#MP_CB').prop('checked', flt.multiPilot);
	if(flt.ifrFlight !=null ) $('#IFR_CB').prop('checked', flt.ifrFlight);
	if(flt.multiEngine !=null ){
		$('#ME_CB').prop('checked', flt.multiEngine);
		$('#SE_CB').prop('checked', !flt.multiEngine);	
	} 
	
	if(flt.totalTime!=null) $('.flight_time .total-time input').val(timeInHrMin(flt.totalTime)).trigger('input');
	if(flt.nightTime!=null) $('.flight_time .night-time input').val(timeInHrMin(flt.nightTime)).trigger('input');
	if(flt.instrumentTime!=null) $('.flight_time .ifr-time input').val(timeInHrMin(flt.instrumentTime)).trigger('input');
	if(flt.crossCountryTime!=null) $('.flight_time .xc-time input').val(timeInHrMin(flt.crossCountryTime)).trigger('input');
	if(flt.picTime!=null) $('.flight_time .pic-time input').val(timeInHrMin(flt.picTime)).trigger('input');
	if(flt.dualTime!=null) $('.flight_time .dual-time input').val(timeInHrMin(flt.dualTime)).trigger('input');
	if(flt.copilotTime!=null) $('.flight_time .copilot-time input').val(timeInHrMin(flt.copilotTime)).trigger('input');
	if(flt.instructorTime!=null) $('.flight_time .instructor-time input').val(timeInHrMin(flt.instructorTime)).trigger('input');


	if(flt.simType != null || flt.simTime != null){
		$('.sim .sim-type input').val(flt.simType);
		if(flt.simTime!=null) $('.sim .sim-time input').val(timeInHrMin(flt.simTime)).trigger('input');
		$('#simulator').prop('checked', true).click();
	}	
